...
import { getPopupAdById, updatePopupAd, deletePopupAd } from "../../models/PopupsModel";
...
const EditPopupAd = () => {
    ...
    useEffect(() => {
        const fetchAd = async () => {
            try {
                ...
                setMenu(menuData);
                const data = await getPopupAdById(id);
                if (data) {
    ...
    const handleSave = async (e) => {
        try {
            ...
            const res = updatePopupAd(updatedBanner);
            toast.promise(
                res,
                {
            ...
            navigate("/dashboard/banners/popup-ads")
        } catch (error) {
    ...
    const handleDelete = async (e) => {
        e.preventDefault();
        try {
            const response = await deletePopupAd(ad.id);
            if (response.status) {
                navigate("/dashboard/banners/popup-ads")
    ...
    return (
        <section className="flex flex-col gap-2 w-full h-full px-[24px] pt-4">
            <div className="flex font-medium text-xl items-center w-full">
                <ArrowBackIosIcon fontSize="small" className="cursor-pointer" onClick={()=>{navigate("/dashboard/banners/popup-ads")}} />
                <h2>Edit Pop Up Ad</h2>
            </div>
