...
const ItemCard = ({ id, name, type, active, onClick, checked, onChange, categoryName, disabled = false }) => {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`flex rounded-xl justify-between items-center h-[66px] overflow-hidden relative text-[#5C5C7A] transition-all duration-300 cursor-pointer hover:bg-[#F8F7FA] border-[1px] border-[#E8E6ED] shadow-xs ${active ? "bg-[#EEEBFA] hover:bg-[#EEEBFA]" : "bg-white "
        } ${isDragging ? "shadow-lg" : ""}`}
      onClick={onClick}
    >
      {/* Left Side */}
      <div className="flex gap-3 px-3 py-3 items-center justify-center flex-1 min-w-0" style={{ cursor: disabled ? "default" : "grab", zIndex:50 }} >
        {!disabled && (
          <div
            {...attributes}
            {...listeners}
        ...
        <div className={`flex flex-col justify-center cursor-pointer h-[42px] min-w-0 flex-1 ${active ? "text-[#4B21E2] " : "text-[#5C5C7A]"} ${disabled ? "ml-0" : ""}`} onClick={onClick}>
          <h1 className="text-[14px] leading-tight truncate" title={name}>{formatName(name)}</h1>
          {categoryName && (
            <span className="text-[12px] text-[#5C5C7A] opacity-70 leading-tight truncate" title={categoryName}>{formatName(categoryName)}</span>
          )}
        </div>
      </div>
      ...
      {/* Type Indicator */}
      <div className={`absolute right-0 top-0 h-full w-4 ${getTypeColor(type)}`} onClick={onClick} />
    </div>
  );
};
